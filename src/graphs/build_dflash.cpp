...
ggml_cgraph * llm_build_context::build_dflash_kv_cache() {
    ...
    if (hparams.dflash_laguna) {
        ...
        target_features = ggml_rms_norm(ctx0, target_features, hparams.f_norm_rms_eps);
        target_features = ggml_mul(ctx0, target_features, model.dflash_aux_norm);
        target_features = ggml_reshape_2d(ctx0, target_features, n_target_features, update_rows);
        cb(target_features, "dflash_kv_aux_norm", -1);
    }
...
ggml_cgraph * llm_build_context::build_dflash() {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (model.layers[il].wqkv_gate != nullptr) {
            ggml_tensor * gate = llm_build_lora_mm(lctx, ctx0, model.layers[il].wqkv_gate, input_normed);
            gate = ggml_softplus(ctx0, gate);
            if (model.layers[il].wqkv_gate->ne[1] == n_head) {

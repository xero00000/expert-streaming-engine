...
common_speculative * common_speculative_init(
    ...
    if (needs_draft_ctx) {
        ...
        if (has_dflash_stage) {
            ...
            // A DFlash decode contains one seed token plus at most block_size - 1
            // masked draft tokens. Keeping the generic draft batch equal to the
            // full cross-context reserves a logits arena for hundreds of tokens
            // (about 208 MiB for Laguna's 100k vocabulary), even though that arena
            // can never be used. The target-feature KV update is built as its own
            // graph and still accepts the complete cross-context window.

...
void ggml_cuda_cpy(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, ggml_tensor * src1, bool disable_indirection_for_this_node) {
    ...
    } else if (src0->type == GGML_TYPE_F32 && ggml_cuda_turbo_kv_type_supported(src1->type)) {
        ggml_cuda_turbo_kv_quantize(ctx, src0, src1);
...
void* ggml_cuda_cpy_fn(const ggml_tensor * src0, ggml_tensor * src1) {
    ...
    if (src0->type == src1->type && ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        ...
        // Native Turbo quantization does not yet implement CUDA-graph
        // destination-pointer indirection. A null function disables capture
        // for this graph while keeping the operation on CUDA.
        return nullptr;
    } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_Q8_0) {

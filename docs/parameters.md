...
| `--expert-ram-cache-mib N` | Bound the native expert RAM cache to N MiB | 0 (disabled) | Selected expert components are read through immutable checked descriptors and retained by lease. This is a hard logical resident-byte limit, independent of the OS page cache. |
| `--expert-ram-staging-mib N` | Bound the reusable storage-read staging arena | `max(largest component, min(cache, 64))` | Must be large enough for one expert component and no larger than the RAM cache; invalid bounds fail during model loading. |
| `--expert-storage-backend mmap\|pread\|io_uring` | Select the exact expert storage backend | `pread` | Backend selection is truthful: unavailable `io_uring` fails explicitly and never silently becomes `pread`. |
| `--expert-sidecar-only` | Require every selected expert computation/upload to use a lease | off | Assertion mode for validation. A missing, incorrectly sized, or event-unsafe lease is fatal; original expert tensors are not a fallback. Requires a non-zero RAM cache. |

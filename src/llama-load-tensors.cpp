...
struct create_tensors_helper : public create_tensors_helper_interface {
    ...
    bool create_maple_tensors(const LLM_TN & tn);
...
bool create_tensors_helper::create_maple_tensors(const LLM_TN & tn) {
    // DeepGrove Maple-Preview — same tensor layout as Mellum/Qwen3MoE with q/k norms + routed experts.
    LOADING_PRELUDE
    model.tok_embd = create_tensor(ctx_input, tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});
...
bool create_tensors_helper::create_dflash_tensors(const LLM_TN & tn) {
    ...
    const bool use_split_ctx = model.split_mode == LLAMA_SPLIT_MODE_GRAPH || model.split_mode == LLAMA_SPLIT_MODE_ATTN;
    const bool is_standard_dflash = ml.get_arch_name() == "dflash";
    ...
    model.output_mtp = model.output;
    if (is_standard_dflash) {
        model.dflash_fc = create_tensor(ctx_output, "fc.weight", {(int64_t) hparams.dflash_n_target_features, n_embd}, 0);
        model.dflash_aux_norm = create_tensor(ctx_output, "enc.aux_norm.weight", {n_embd, (int64_t) hparams.dflash_n_target_layers}, 0);
        model.dflash_hidden_norm = create_tensor(ctx_output, "enc.output_norm.weight", {n_embd}, 0);
    } else {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        layer.attn_norm = create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, 0);
        layer.attn_post_norm = is_standard_dflash
            ? create_tensor(ctx_split, format("blk.%d.ffn_norm.weight", i), {n_embd}, 0)
            : create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_POST_NORM, "weight", i), {n_embd}, 0);
        ...
        if (is_standard_dflash) {
            const std::string gate_name = format("blk.%d.attn_gate.weight", i);
            const ggml_tensor * gate_meta = ml.get_tensor_meta(gate_name.c_str());
            int64_t gate_width = n_head;
            if (gate_meta != nullptr && gate_meta->ne[1] == n_embd_head_v * n_head) {
                gate_width = n_embd_head_v * n_head;
            }
            layer.wqkv_gate = create_tensor(ctx_split, gate_name, {n_embd, gate_width}, 0);
        }
...
bool create_tensors_helper::create_tensors() {
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_MAPLE:
            use_mmap_buffer = create_maple_tensors(tn); break;
